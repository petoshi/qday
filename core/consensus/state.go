...
type Network struct {
	Qday *QdayParams `json:"qday,omitempty"`
...
func (s State) MaxFutureTimestamp(currentTime time.Time) time.Time {
	// A QDAY release may be distributed before its scheduled genesis. Until
	// that instant there is no timestamp that can satisfy both this policy and
	// the consensus median timestamp inherited from genesis, so honest nodes
	// cannot accept or relay a first block early.
	if s.Network.Qday != nil && s.childHeight() == 1 && currentTime.Before(s.Network.HardforkOak.GenesisTimestamp) {
		return currentTime
	}
...
func (s State) BlockReward() types.Currency {
	if p := s.Network.Qday; p != nil {
		if s.childHeight() == 0 || s.childHeight() > p.MiningBlocks {
...
func (s State) InputSigHash(txn types.V2Transaction) types.Hash256 {
	if s.Network.Qday != nil {
		return hashAll("qday/sig/input/v1", s.Network.Qday.Domain, types.V2TransactionSemantics(txn))
	}
