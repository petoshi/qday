...
func (ms *MidState) createSiacoinElement(id types.SiacoinOutputID, sco types.SiacoinOutput) *SiacoinElementDiff {
	...
	sced.Created = true
	if ms.base.Network.Qday != nil {
		sced.SiacoinElement.MaturityHeight = ms.base.childHeight()
	}
	return sced
}
...
func (ms *MidState) ApplyV2Transaction(txn types.V2Transaction) {
	if ms.base.Network.Qday != nil {
		if e, err := ParseQdayEnvelope(txn.ArbitraryData); err == nil && e.Kind == QdayCanaryProof {
			ms.qdayProved = true
		}
	}
	txid := txn.ID()
...
func ApplyBlock(s State, b types.Block, bs V1BlockSupplement, targetTimestamp time.Time) (State, ApplyUpdate) {
	...
	if s.Network.Qday != nil && s.QdayHeight == 0 && ms.qdayProved {
		s.QdayHeight = s.Index.Height + s.Network.Qday.ActivationDelay
	}
