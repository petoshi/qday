...
func (p SpendPolicy) Verify(height uint64, medianTimestamp time.Time, sigHash Hash256, sigs []Signature, preimages [][32]byte) error {
	...
	verify = func(p SpendPolicy) error {
		...
		case PolicyTypeSLHDSA:
			if len(preimages) < QdaySignatureChunks || !verifyQdayReserve(p, sigHash, preimages[:min(len(preimages), QdaySignatureChunks)]) {
				return errors.New("invalid SLH-DSA reserve signature")
			}
			preimages = preimages[QdaySignatureChunks:]
			return nil
		case PolicyTypeHash:
...
func ParseSpendPolicy(s string) (SpendPolicy, error) {
	...
	parseSpendPolicy = func() SpendPolicy {
		...
		case "slh":
			return PolicySLHDSA(parsePubkey())
		case "h":
