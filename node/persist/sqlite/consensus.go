...
func (ut *updateTx) ApplyIndex(index types.ChainIndex, state wallet.AppliedState) error {
	...
	if err := addSiacoinElements(tx, state.CreatedSiacoinElements, indexID, index.Height, ut.indexMode, log.Named("addSiacoinElements")); err != nil {
...
func addSiacoinElements(tx *txn, elements []wallet.CreatedSiacoinElement, indexID int64, height uint64, indexMode wallet.IndexMode, log *zap.Logger) error {
	if len(elements) == 0 {
		return nil
	}
	...
	for _, se := range elements {
		...
		_, err = insertStmt.Exec(encode(se.ID), encode(se.SiacoinOutput.Value), encode(se.StateElement.MerkleProof), se.StateElement.LeafIndex, se.MaturityHeight, addrRef.ID, se.MaturityHeight <= height, indexID, se.Origin.Source, encode(se.Origin.ID), se.Origin.Index)
		if err != nil {
			return fmt.Errorf("failed to execute statement: %w", err)
		}
