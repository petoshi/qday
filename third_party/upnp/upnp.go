...
func (d Device) Forward(port uint16, proto string, desc string) error {
	return d.ForwardLease(port, proto, desc, 0)
}

// ForwardLease requests a renewable mapping in seconds (zero means permanent).
func (d Device) ForwardLease(port uint16, proto, desc string, lease uint32) error {
...
func (d Device) IsForwarded(port uint16, proto string) bool {
	...
	return resp.NewEnabled && resp.NewInternalClient == d.internalIP && resp.NewInternalPort == port
}
...
func getInternalIP(loc string) (string, error) {
	...
	devAddr, err := net.ResolveUDPAddr("udp4", net.JoinHostPort(baseURL.Hostname(), port))
	if err != nil {
		return "", err
	}
...
func doDiscoverAll(locations <-chan string, devices chan<- Device) {
	...
	for url := range locations {
		...
		go func(url string) {
			...
			defer cancel()
			if _, err := getInternalIP(url); err != nil {
				return
			}
...
type Error = goupnp.Error

// PortMapping describes the complete rule, including its owner description.
type PortMapping = goupnp.GetSpecificPortMappingEntryResponse

// Mapping returns the rule for the external port, or UPnP error 714 if absent.
func (d Device) Mapping(port uint16, proto string) (PortMapping, error) {
	return d.client.GetSpecificPortMappingEntry(goupnp.GetSpecificPortMappingEntryRequest{NewExternalPort: port, NewProtocol: proto})
}

// InternalIP returns the LAN address used as the mapping destination.
func (d Device) InternalIP() string { return d.internalIP }

// WithContext binds subsequent router requests to ctx.
func (d Device) WithContext(ctx context.Context) Device {
	d.client = d.client.WithContext(ctx)
	return d
}
